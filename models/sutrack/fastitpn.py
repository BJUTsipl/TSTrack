...
class Fast_iTPN(nn.Module):
    def __init__(self, search_size=224,template_size=112, patch_size=16, in_chans=3, embed_dim=512, depth_stage1=3, depth_stage2=3, depth=24,
        ...
        # 初始化DAM
        # 在Fast_iTPN的__init__方法中修改DAM初始化部分
        if kwargs.get('use_dam', False):
            assert kwargs.get('in_chans', 3) == 3, "DAM模块需要RGB输入"

            # 直接实例化（不再读取dam_reduction参数）
            self.dam_template = LightDAM()  # 固定reduction_ratio=2
            self.dam_search = LightDAM()    # 固定reduction_ratio=2
    ...
    def prepare_tokens_with_masks(self, template_list, search_list, template_anno_list):
        #1. DAM预处理
        template_list = [self.dam_template(x) for x in template_list]
        search_list = [self.dam_search(x) for x in search_list]
...
def load_pretrained(model, checkpoint, pos_type, patchembed_init):
    ...
    patch_embedding_weight_pretrained = state_dict['patch_embed.proj.weight'].float()
    if patchembed_init == "copy":
        if model.patch_embed.proj.weight.shape[1] == 3:  # 如果是3通道模型
            model.patch_embed.proj.weight.data = patch_embedding_weight_pretrained  # 直接赋值
    else:
        raise NotImplementedError('cfg.MODEL.ENCODER.PATCHEMBED_INIT must be choosen from copy')
    ...
    for name, param in model.named_parameters():
        if param.numel() == 0:
            continue  # 跳过空张量
            # 适配优化后的参数命名
        if 'dam_' in name.lower():
            # 空间注意力部分（深度可分离卷积）
            if 'spatial_attn.depthwise.weight' in name:
                # Dirac初始化保持GMP/GAP特性
                with torch.no_grad():
                    if '0:1' in name:  # GMP路径
                        param.data.fill_(1.0)
                    elif '1:2' in name:  # GAP路径
                        param.data.fill_(1.0)
            elif 'spatial_attn.pointwise.weight' in name:
                nn.init.constant_(param, 0.5)  # 平衡权重初始化

            # 通道注意力部分（1x1卷积）
            elif 'channel_attn.conv.0.weight' in name:  # 3->1层
                nn.init.eye_(param)  # 单位矩阵初始化
            elif 'channel_attn.conv.2.weight' in name:  # 1->3层
                nn.init.eye_(param)  # 单位矩阵初始化
...
@register_model
def fastitpnt(pretrained=True, pos_type="interpolate", pretrain_type="pretrained/itpn/fast_itpn_tiny_1600e_1k.pt", patchembed_init="copy", **kwargs):
    model = Fast_iTPN(
        patch_size=16, in_chans=3, embed_dim=384, depth_stage1=1, depth_stage2=1, depth=8, num_heads=6,
        bridge_mlp_ratio=3.,
...
def fastitpns(pretrained=False, pos_type="interpolate", pretrain_type="", patchembed_init="copy", **kwargs):
    model = Fast_iTPN(
        patch_size=16, in_chans=3, embed_dim=384, depth_stage1=2, depth_stage2=2, depth=20, num_heads=6, bridge_mlp_ratio=3.,
        mlp_ratio=3., qkv_bias=True, norm_layer=partial(nn.LayerNorm, eps=1e-6),
        convmlp=True,
        naiveswiglu=True,
        subln=True,
...
def fastitpnb(pretrained=False, pos_type="interpolate", pretrain_type="", patchembed_init="copy", **kwargs):
    model = Fast_iTPN(
        patch_size=16, in_chans=3, embed_dim=512, depth_stage1=3, depth_stage2=3, depth=24, num_heads=8, bridge_mlp_ratio=3.,
        mlp_ratio=3., qkv_bias=True, norm_layer=partial(nn.LayerNorm, eps=1e-6),
        convmlp=True,
        naiveswiglu=True,
        subln=True,
...
def fastitpnl(pretrained=False, pos_type="interpolate", pretrain_type="", patchembed_init="copy", **kwargs):
    model = Fast_iTPN(
        patch_size=16, in_chans=3, embed_dim=768, depth_stage1=2, depth_stage2=2, depth=40, num_heads=12, bridge_mlp_ratio=3.,
        mlp_ratio=3., qkv_bias=True, norm_layer=partial(nn.LayerNorm, eps=1e-6),
        convmlp=True,
        naiveswiglu=True,
        subln=True,
